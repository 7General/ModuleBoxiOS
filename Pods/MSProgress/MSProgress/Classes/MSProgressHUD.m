...
#define MS_AUTORELEASE(exp) exp
#define MS_RELEASE(exp) exp
#define MS_RETAIN(exp) exp
#else
#define MS_AUTORELEASE(exp) [exp autorelease]
#define MS_RELEASE(exp) [exp release]
#define MS_RETAIN(exp) [exp retain]
#endif
...
#define MS_TEXTSIZE(text, font) [text length] > 0 ? [text \
sizeWithAttributes:@{NSFontAttributeName:font}] : CGSizeZero;
#else
#define MS_TEXTSIZE(text, font) [text length] > 0 ? [text sizeWithFont:font] : CGSizeZero;
#endif

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
#define MS_MULTILINE_TEXTSIZE(text, font, maxSize, mode) [text length] > 0 ? [text \
...
@property (atomic, MS_STRONG) UIView *indicator;
@property (atomic, MS_STRONG) NSTimer *graceTimer;
@property (atomic, MS_STRONG) NSTimer *minShowTimer;
@property (atomic, MS_STRONG) NSDate *showStarted;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [MSProgressHUD allHUDsForView:view];
    for (MSProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hide:animated];
    }
    return [huds count];
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animationType = MSProgressHUDAnimationFade;
        //		self.mode = MSProgressHUDModeIndeterminate;
        self.mode = MSProgressHUD25X;
...
- (void)show:(BOOL)animated {
    NSAssert([NSThread isMainThread], @"MSProgressHUD needs to be accessed on the main thread.");
...
- (void)showUsingAnimation:(BOOL)animated {
    ...
    if (animated && animationType == MSProgressHUDAnimationZoomIn) {
        self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
    } else if (animated && animationType == MSProgressHUDAnimationZoomOut) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == MSProgressHUDAnimationZoomIn || animationType == MSProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
    ...
    if (animated && showStarted) {
        ...
        // in the done method
        if (animationType == MSProgressHUDAnimationZoomIn) {
            self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
        } else if (animationType == MSProgressHUDAnimationZoomOut) {
            self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
        }
...
- (void)showWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated {
    ...
    targetForExecution = MS_RETAIN(target);
    objectForExecution = MS_RETAIN(object);
...
- (void)updateIndicators {
    ...
    if (mode == MSProgressHUDModeIndeterminate) {
        if (!isActivityIndicator) {
            // Update to indeterminate indicator
            [indicator removeFromSuperview];
            self.indicator = MS_AUTORELEASE([[UIActivityIndicatorView alloc]
    ...
    else if (mode == MSProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        self.indicator = MS_AUTORELEASE([[MBBarProgressView alloc] init]);
        [self addSubview:indicator];
    }
    else if (mode == MSProgressHUDModeDeterminate || mode == MSProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            self.indicator = MS_AUTORELEASE([[MBRoundProgressView alloc] init]);
            [self addSubview:indicator];
        }
        if (mode == MSProgressHUDModeAnnularDeterminate) {
    ...
    else if (mode == MSProgressHUDModeCustomView && customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        self.indicator = customView;
        [self addSubview:indicator];
    } else if (mode == MSProgressHUDModeText) {
        [indicator removeFromSuperview];
        self.indicator = nil;
    }
...
- (void)layoutSubviews {
    ...
    if(mode == MSProgressHUD25X){
        ...
        /**标题*/
        CGSize labelSize = MS_TEXTSIZE(label.text, label.font);
        labelSize.width = MIN(labelSize.width, maxWidth);
        ...
        CGFloat remainingHeight = bounds.size.height - totalSize.height - kPadding - 4 * margin;
        CGSize maxSize = CGSizeMake(maxWidth, remainingHeight);
        CGSize detailsLabelSize = MS_MULTILINE_TEXTSIZE(detailsLabel.text, detailsLabel.font, maxSize, detailsLabel.lineBreakMode);
        totalSize.width = MAX(totalSize.width, detailsLabelSize.width);
        totalSize.height += detailsLabelSize.height;
